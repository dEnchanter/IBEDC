...
const presets = {
  color: {
    ...
    brandPurple: 'bg-purple-700 border-purple-700 hover:bg-purple-800 focus:ring-purple-400',
    cancel: 'bg-gray-200 hover:bg-gray-300 focus:ring-gray-400 hover:bg-opacity-80',
  },
  hollowColor: {
    brandLemon: {
...
export function IconButton({
  ...
  return (
    <Button border={`${roundClass}`} padding={padding} {...rest} innerRef={innerRef}>
      {Icon && <Icon />}
    </Button>
  );
}
...
export function Button({
  ...
  loadingMargin = 'ml-2',
  disabled,
  border = 'border focus:ring-opacity-30 focus:ring-ibedc-brand-orange focus:ring-2 rounded',
  look = 'brandPurple',
  className,
  onClick,
  innerRef,
  ...
  let _className =
    className ||
    `focus:outline-none ${font} bg-ibedc-brand-orange ${border} ${padding} ${margin} ${width} ${disabledClass}`;
...
export function HollowButton({
  ...
  let Icon = icon;
  let disabledClass = disabled ? ' cursor-not-allowed' : '';
  let preset = presets.hollowColor[look];
      ...
      <Modal show={showConfirm} title={title} width="w-96">
        <div>
          ...
          <div className="flex justify-end mt-4">
            <Button onClick={accept} padding="py-1 px-4" margin="mr-2" look="brandOrange">
              {acceptLabel}
            </Button>
